...
(C++/GLSL), ~/Projects/resolume/macroblock, not yet on GitHub, v0.1.0
unreleased.*
...
- `tools/verify.sh` is 19 checks and all of them pass on a clean universal
...
**`source/StoatworksAbout.h` is hand-written here.** It is normally generated by
`stoatworks-backend/scripts/sync-about.py` from the website's `projects.json`,
and there is no `macroblock` entry there yet. Add one and re-run the sync before
release, or the next sync of any sibling repo will not know this file is meant
to exist.

## Still to do

- Run it in Resolume on real content, with real audio. That is the first thing.
- Run the OpenFX build in Resolve.
- `projects.json` entry + `sync-about.py`, then `sync.sh` once its paths are
  fixed.
- Public repo, release, demo deploy. None of that has happened.
